...
if __name__ == "__main__":
       ...
       security_Dict = {x:df_security.select(x+'.foodInsecurity').first()['foodInsecurity'] for x in df_security.columns}
       ...
       #filter by UPC
       list_UPCcode = sample_df.select("code").rdd.flatMap(lambda x: x).collect()
       ...
       df = df.withColumn('foodInsecurity', F.col('foodInsecurity').cast('integer'))
