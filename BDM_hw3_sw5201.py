...
if __name__ == "__main__":
       ...
       security_Dict = { x:df_store.select(x+'.foodInsecurity').first()['foodInsecurity'] for x in df_store.columns}     
       ...
       UDF_double = F.udf(lambda x: insecurityDict[x], T.DoubleType())
       df = df.withColumn('foodInsecurity', callnewColsUdf(F.col('store')) ) #join the security data to df

       #filter by UPC
       list_UPCcode = df_sample.select("code").rdd.flatMap(lambda x: x).collect()
       df = df.filter(F.col("code").isin(list_UPCcode))
